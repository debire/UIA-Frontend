...
//         // Uncomment below to send order update email when status changes to shipped
//         // This calls POST /send-order-update/{order_id} which sets status to shipped and sends email
...
//       const errorMessage = err.response?.data?.detail || err.message || 'Failed to update shipping info';
...
//       return status === 'pending' || status === 'processing' || status === 'unpaid' || status === 'ordered';
//     }).length;

//     const completedOrders = orders.filter(order => {
//       const status = (order.status || '').toLowerCase();
//       return status === 'completed' || status === 'delivered' || status === 'paid' || status === 'succeeded' || status === 'shipped';
//     }).length;
...
//   // Zustand store
//   const { orders, fetchAllOrders, loading, deleteOrder } = useAdminOrderStore();
...
//       // Update order status locally (the backend /send-order-update endpoint sends email and times out)
//       // For now, just update the local state - backend needs an endpoint to update status without sending email
//       if (shippingFormData.orderStatus) {
//         setSelectedOrder(prev => ({
//           ...prev,
//           status: shippingFormData.orderStatus
...
//       // Refresh the orders list to update status in the table
//       fetchAllOrders();
